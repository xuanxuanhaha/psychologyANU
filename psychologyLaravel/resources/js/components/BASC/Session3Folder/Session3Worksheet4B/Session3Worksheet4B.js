...
import styles from './session3worksheet4b.module.css';
...
const closeModal = () => {
  ...
  useEffect(() => {
    ...
    if(userData){
        ...
        axios.get(`/api/sessionresponse/3?userid=${userData.userid}&&questionno=session3worksheet4b`)
          .then(response => {
            if(response.data){
                if(response.data.sessionresponse.response){
                    const questionanswer = JSON.parse(response.data.sessionresponse.response)
  ...
  const passData = () => {
    const data = {
        ...
        'sessionid': 3,
        'questionno': 'session3worksheet4b',
  ...
  const downloadCSV = () => {
    ...
    link.setAttribute('download', 'Session3Worksheet4B.xlsx');
          ...
          <Typography title={'content'} position={'left'}  contentwidth={'100%'}>
              {
                ...
                <p>Select another 
                    <b onClick={openDialogWithTemplateRef}>
                     ...
                     and plan your activities to achieve the goal that you want to practice soon or the weeks after the next.</p>
                :
...
export default Session3Worksheet4B;