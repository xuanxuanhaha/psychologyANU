...
import styles from './session1worksheet2q1b.module.css';
...
const Session1worksheet2Q1B = () => {
  ...
  useEffect(() => {
    ...
    if(userData){
        ...
        axios.get(`/api/sessionresponse/1?userid=${userData.userid}&&questionno=session1worksheet2q1b`)
        .then(response => {
        if(response.data){
            if(response.data.sessionresponse.response){
            const questionanswer = JSON.parse(response.data.sessionresponse.response)
  ...
  const passData = () => {
    ...
    const data = {
        ...
        'sessionid': 1,
        'questionno': 'session1worksheet2q1b',
...
export default Session1worksheet2Q1B;