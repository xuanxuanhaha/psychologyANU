...
import styles from './session1worksheetq1b.module.css';
...
const Session1WorksheetQ1B = () => {
  ...
  useEffect(() => {
    ...
    if(userData){
        axios.get(`/api/sessionresponse/1?userid=${userData.userid}&&questionno=session1WorksheetQ1B`)
          .then(response => {
            if(response.data){
  ...
  const passData = () => {
    ...
    const data = {
        ...
        'sessionid': 1,
        'questionno': 'session1WorksheetQ1B',
...
export default Session1WorksheetQ1B;