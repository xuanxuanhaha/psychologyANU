...
import styles from './session4preb.module.css';
...
const Session4PreB = () => {
  ...
  useEffect(() => {
    ...
    if(userData){
        axios.get(`/api/sessionresponse/4?userid=${userData.userid}&&questionno=session4preb`)
          .then(response => {
            if(response.data){
