...
import styles from './session4worksheet1b.module.css';
...
const Session4Worksheet1B = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/sessionresponse/4?userid=${userData.userid}&&questionno=session4worksheet1b`)
    .then(response => {
    if(response.data){
        if(response.data.sessionresponse.response){
            const questionanswer = JSON.parse(response.data.sessionresponse.response)
  ...
  const passData = () => {
    ...
    const data = {
        ...
        'sessionid': 4,
        'questionno': 'session4worksheet1b',
...
export default Session4Worksheet1B;