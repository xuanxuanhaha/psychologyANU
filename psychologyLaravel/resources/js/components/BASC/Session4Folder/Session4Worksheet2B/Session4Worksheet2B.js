...
import styles from './session4worksheet2b.module.css';
...
const Session4Worksheet2B = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/sessionresponse/1?userid=${userData.userid}&&questionno=session1worksheetq2b`).then(response => {
        if(response.data){
          if(response.data.sessionresponse.response){
            const questionanswer = JSON.parse(response.data.sessionresponse.response)
    ...
    axios.get(`/api/sessionresponse/4?userid=${userData.userid}&&questionno=session4worksheet2b`)
    .then(response => {
    if(response.data){
        if(response.data.sessionresponse.response){
            const questionanswer = JSON.parse(response.data.sessionresponse.response)
  ...
  const passData = () => {
    ...
    const data = {
        ...
        'sessionid': 4,
        'questionno': 'session4worksheet2b',
...
export default Session4Worksheet2B;