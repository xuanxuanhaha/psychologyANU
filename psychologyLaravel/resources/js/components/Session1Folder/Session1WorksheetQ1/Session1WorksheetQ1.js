...
import styles from './session1worksheetq1.module.css';
...
const Session1WorksheetQ1 = () => {
  ...
  useEffect(() => {
    ...
    if(userData){
        axios.get(`/api/sessionresponse/1?userid=${userData.userid}&&questionno=session1WorksheetQ1`)
          .then(response => {
            if(response.data){
  ...
  const passData = () => {
    ...
    const data = {
        ...
        'sessionid': 1,
        'questionno': 'session1WorksheetQ1',
...
export default Session1WorksheetQ1;