...
import styles from './session1worksheetq2.module.css';
...
const Session1WorksheetQ2 = () => {
  ...
  useEffect(() => {
    ...
    if(userData){
        ...
        axios.get(`/api/sessionresponse/1?userid=${userData.userid}&&questionno=session1WorksheetQ2`)
          .then(response => {
            if(response.data){
              if(response.data.sessionresponse.response){
                const questionanswer = JSON.parse(response.data.sessionresponse.response)
  ...
  const passData = () => {
    ...
    const data = {
        ...
        'sessionid': 1,
        'questionno': 'session1WorksheetQ2',
...
export default Session1WorksheetQ2;