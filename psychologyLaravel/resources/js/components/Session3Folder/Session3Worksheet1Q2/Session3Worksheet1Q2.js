...
const Session3Worksheet1Q2 = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/sessionresponse/4?userid=${userData.userid}&&questionno=session3Worksheet1Q2`)
    .then(response => {
    if(response.data){
        if(response.data.sessionresponse.response){
            const questionanswer = JSON.parse(response.data.sessionresponse.response)
       ...
       <Navbar />
        <div className={styles.background_image} id="background_image">
            <div className={styles.whole_border}>
                ...
                <ProgressBar percentageNo={68} language={language} />
                ...
                <br />
                {
                    ...
                    <React.Fragment>
                        <Typography title={'content'} position={'left'} contentwidth={'100%'}>
                            <i>In the next session we will practice community connection, kindness to all, and mindfulness. </i>
                        </Typography>
                    </React.Fragment>
                    :
                    <React.Fragment>
