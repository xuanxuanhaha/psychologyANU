...
import styles from './session3review.module.css';
...
const Session3Review = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/sessionresponse/3?userid=${userData.userid}&&questionno=session3Review`)
    .then(response => {
      if(response.data){
          if(response.data.sessionresponse.response){
              const questionanswer = JSON.parse(response.data.sessionresponse.response)
  ...
  const jumptonextpage = () => {
    navigate(`/session3worksheet1start`);
  };
    ...
    <div>
      ...
      <div className={styles.background_image} id="background_image">
        <div className={styles.whole_border}>
          ...
          <ProgressBar percentageNo={14} language={language} />
