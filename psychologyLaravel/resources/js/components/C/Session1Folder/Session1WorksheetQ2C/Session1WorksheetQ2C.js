...
import styles from './session1worksheetq2c.module.css';
...
const Session1WorksheetQ2C = () => {
  ...
  useEffect(() => {
    ...
    if(userData){
        axios.get(`/api/sessionresponse/1?userid=${userData.userid}&&questionno=session1WorksheetQ1C`).then(response => {
          if(response.data){
        ...
        axios.get(`/api/sessionresponse/1?userid=${userData.userid}&&questionno=session1WorksheetQ2C`)
          .then(response => {
            if(response.data){
              if(response.data.sessionresponse.response){
                const questionanswer = JSON.parse(response.data.sessionresponse.response)
  ...
  const passData = () => {
    ...
    const data = {
        ...
        'sessionid': 1,
        'questionno': 'session1WorksheetQ2C',
...
export default Session1WorksheetQ2C;